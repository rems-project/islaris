...
Definition a18 : list trc := [
  [
    ...
    Assume (AExp_Binop (Eq) (AExp_Manyop (Bvmanyarith Bvand) [AExp_Manyop (Bvmanyarith Bvadd) [AExp_Val (AVal_Var "R1" []) Mk_annot; AExp_Val (AVal_Bits [BV{64%N} 0x1%Z]) Mk_annot] Mk_annot; AExp_Val (AVal_Bits [BV{64%N} 0xfff0000000000000%Z]) Mk_annot] Mk_annot) (AExp_Val (AVal_Bits [BV{64%N} 0x0%Z]) Mk_annot) Mk_annot) Mk_annot;
    Smt (Assert (Binop (Eq) (Manyop (Bvmanyarith Bvand) [Manyop (Bvmanyarith Bvadd) [Val (Val_Symbolic 29%Z) Mk_annot; Val (Val_Bits [BV{64%N} 0x1%Z]) Mk_annot] Mk_annot; Val (Val_Bits [BV{64%N} 0xfff0000000000000%Z]) Mk_annot] Mk_annot) (Val (Val_Bits [BV{64%N} 0x0%Z]) Mk_annot) Mk_annot)) Mk_annot;
    ...
    Smt (DeclareConst 1316%Z (Ty_BitVec 56%N)) Mk_annot;
    Smt (DefineConst 1325%Z (Unop (ZeroExtend 8%N) (Manyop Concat [Val (Val_Bits [BV{4%N} 0x0%Z]) Mk_annot; Unop (Extract 51%N 0%N) (Val (Val_Symbolic 87%Z) Mk_annot) Mk_annot] Mk_annot) Mk_annot)) Mk_annot;
    Smt (DeclareConst 1326%Z (Ty_BitVec 8%N)) Mk_annot;
    ReadMem (RegVal_Base (Val_Symbolic 1326%Z)) (RegVal_Base (Val_Enum ((Mk_enum_id 6%nat), Mk_enum_ctor 0%nat))) (RegVal_Base (Val_Symbolic 1325%Z)) 1%N None Mk_annot;
    Smt (DefineConst 1330%Z (Manyop Concat [Val (Val_Bits [BV{32%N} 0x0%Z]) Mk_annot; Manyop Concat [Val (Val_Bits [BV{24%N} 0x0%Z]) Mk_annot; Val (Val_Symbolic 1326%Z) Mk_annot] Mk_annot] Mk_annot)) Mk_annot;
    WriteReg "R0" [] (RegVal_Base (Val_Symbolic 1330%Z)) Mk_annot;
    Smt (DefineConst 1331%Z (Val (Val_Symbolic 87%Z) Mk_annot)) Mk_annot;
    WriteReg "R1" [] (RegVal_Base (Val_Symbolic 1331%Z)) Mk_annot;
    Smt (DeclareConst 1332%Z (Ty_BitVec 64%N)) Mk_annot;
    ReadReg "_PC" [] (RegVal_Base (Val_Symbolic 1332%Z)) Mk_annot;
    Smt (DefineConst 1333%Z (Manyop (Bvmanyarith Bvadd) [Val (Val_Symbolic 1332%Z) Mk_annot; Val (Val_Bits [BV{64%N} 0x4%Z]) Mk_annot] Mk_annot)) Mk_annot;
    WriteReg "_PC" [] (RegVal_Base (Val_Symbolic 1333%Z)) Mk_annot
  ]
].

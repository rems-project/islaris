...
Definition a34 : list trc := [
  [
    ...
    Assume (AExp_Binop (Eq) (AExp_Manyop (Bvmanyarith Bvand) [AExp_Manyop (Bvmanyarith Bvadd) [AExp_Val (AVal_Var "R21" []) Mk_annot; AExp_Manyop (Bvmanyarith Bvmul) [AExp_Val (AVal_Var "R24" []) Mk_annot; AExp_Val (AVal_Bits [BV{64%N} 0x8%Z]) Mk_annot] Mk_annot] Mk_annot; AExp_Val (AVal_Bits [BV{64%N} 0xfff0000000000007%Z]) Mk_annot] Mk_annot) (AExp_Val (AVal_Bits [BV{64%N} 0x0%Z]) Mk_annot) Mk_annot) Mk_annot;
    Smt (Assert (Binop (Eq) (Manyop (Bvmanyarith Bvand) [Manyop (Bvmanyarith Bvadd) [Val (Val_Symbolic 29%Z) Mk_annot; Manyop (Bvmanyarith Bvmul) [Val (Val_Symbolic 30%Z) Mk_annot; Val (Val_Bits [BV{64%N} 0x8%Z]) Mk_annot] Mk_annot] Mk_annot; Val (Val_Bits [BV{64%N} 0xfff0000000000007%Z]) Mk_annot] Mk_annot) (Val (Val_Bits [BV{64%N} 0x0%Z]) Mk_annot) Mk_annot)) Mk_annot;
    ReadReg "R24" [] (RegVal_Base (Val_Symbolic 30%Z)) Mk_annot;
    ReadReg "R21" [] (RegVal_Base (Val_Symbolic 29%Z)) Mk_annot;
    Smt (DefineConst 103%Z (Manyop (Bvmanyarith Bvadd) [Val (Val_Symbolic 29%Z) Mk_annot; Manyop Concat [Unop (Extract 60%N 0%N) (Val (Val_Symbolic 30%Z) Mk_annot) Mk_annot; Val (Val_Bits [BV{3%N} 0x0%Z]) Mk_annot] Mk_annot] Mk_annot)) Mk_annot;
    ReadReg "PSTATE" [Field "EL"] (RegVal_Struct [("EL", RegVal_Base (Val_Bits [BV{2%N} 0x2%Z]))]) Mk_annot;
    Smt (DefineConst 107%Z (Binop (Eq) (Val (Val_Symbolic 103%Z) Mk_annot) (Manyop (Bvmanyarith Bvand) [Val (Val_Symbolic 103%Z) Mk_annot; Val (Val_Bits [BV{64%N} 0xfffffffffffffff8%Z]) Mk_annot] Mk_annot) Mk_annot)) Mk_annot;
    ...
    Smt (Assert (Unop (Not) (Unop (Not) (Val (Val_Symbolic 107%Z) Mk_annot) Mk_annot) Mk_annot)) Mk_annot;
    Smt (Assert (Binop (Eq) (Val (Val_Symbolic 103%Z) Mk_annot) (Manyop (Bvmanyarith Bvand) [Val (Val_Symbolic 103%Z) Mk_annot; Val (Val_Bits [BV{64%N} 0xfffffffffffffff8%Z]) Mk_annot] Mk_annot) Mk_annot)) Mk_annot;
    ...
    Smt (DeclareConst 1303%Z (Ty_BitVec 64%N)) Mk_annot;
    ReadMem (RegVal_Base (Val_Symbolic 1303%Z)) (RegVal_Base (Val_Enum ((Mk_enum_id 7%nat), Mk_enum_ctor 0%nat))) (RegVal_Base (Val_Symbolic 1302%Z)) 8%N None Mk_annot;
    Smt (DefineConst 1307%Z (Val (Val_Symbolic 1303%Z) Mk_annot)) Mk_annot;
    WriteReg "R0" [] (RegVal_Base (Val_Symbolic 1307%Z)) Mk_annot
  ]
].
